...
import           Diagrams.Prelude hiding (hcat, vcat, at)
import           LGtk
import           Prelude
import           Safe (headMay)
...
circleDrawer = do
    ...
    vcat
        [ hcat [ button (pure "Undo") (clearFocusWhenPressed undo)
               , button (pure "Redo") (clearFocusWhenPressed redo)
               ]
        , canvas 480 480 480 mouse Nothing
        ...
        , cell (readRef focused) $ \case
            ...
            Just cur -> do
                ...
                hcat [ hscale 1 100 1 (curRadiusLens `lensMap` state)
