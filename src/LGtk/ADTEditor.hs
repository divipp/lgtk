...
adtEditor :: (MonadRegister m, ExtRef m, ADTLens a, Inner m ~ Inner' m) => Ref m a -> C m (I m)
adtEditor = liftM Action . memoRead . editor  where
    ...
    mkEditors :: (MonadRegister m, ExtRef m, Inner m ~ Inner' m) => Elems xs -> Ref m (Elems xs) -> C m [I m]
